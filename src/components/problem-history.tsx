...
function History() {
  ...
  return <div className="history-list">{problems.map(problem => <article className="history-card" key={problem._id}><div><span className="eyebrow">{new Date(problem._creationTime).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})}</span><h2><Link href={`/problems/${problem._id}`}>{(problem.text || problem.transcript || "A photo or voice-note repair").slice(0,110)}</Link></h2><span className="status-badge">{stateLabels[problem.state] ?? problem.state}</span>{problem.selectedGuideVersionId&&<SelectedGuideSummary guideVersionId={problem.selectedGuideVersionId}/>}</div><Link className="button button-secondary" href={`/problems/${problem._id}`}>Open <ArrowUpRight size={15}/></Link></article>)}</div>;
}
