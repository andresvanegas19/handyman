...
export function GuideContent({ guide, assembly }: { guide: Guide; assembly?: ReviewedAssembly }) {
  return <>
    ...
    <PreviewNotice/>
    {guide.status === "draft" && <div className="notice notice-sage"><ShieldAlert size={18}/><p><strong>For exploration, not repair instructions.</strong>This is a draft example. A qualified reviewer must approve applicability and safety before you use it.</p></div>}
    <div className="guide-title"><span className="eyebrow">{guide.category}</span><h1>{guide.title}</h1><p>{guide.summary}</p>
    ...
    <VisualTutorial guide={guide} assembly={assembly}/>
