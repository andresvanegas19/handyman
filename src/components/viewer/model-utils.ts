...
export function resolvePartNodes(root: Object3D, parts: AssemblyPart[]): Map<string, Object3D[]> {
  ...
  if (!parts.length) throw new Error("This assembly has no reviewed part labels.");
  for (const part of parts) {
    ...
    for (const name of part.nodeNames) {
      ...
      if (!found) throw new Error(`The reviewed model is missing the node “${name}” for ${part.label}.`);
    }
    ...
    for (const node of roots) {
      node.traverse((child) => {
        ...
        if (existing && existing !== part.id) throw new Error("Reviewed part mappings overlap. Each mesh must belong to one part.");
